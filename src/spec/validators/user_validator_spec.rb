...
RSpec.describe UserValidator do
  describe '.validate' do
    shared_examples 'rejects invalid format' do |input|
      it "rejects" do
        result = UserValidator.validate(input)
        expect(result[:valid]).to eq(false)
        expect(result[:error]).to eq('User id must be an integer.')
    ...
    context 'with valid format but non-existent user' do
      before { allow(User).to receive(:exists?).with(-1).and_return(false) }

      it 'returns a status 404' do
        result = UserValidator.validate(-1)
        expect(result[:valid]).to eq(false)
        expect(result[:error]).to eq('The user does not exist.')
        expect(result[:status]).to eq(:not_found)
      end
    end

    context 'with valid format and user exists' do
      let!(:user1) { create(:user, id: 123) }

      it 'passes all validation' do
        expect(UserValidator.validate(123)).to eq({ valid: true })
      end
    end
  end
end